...
public:
   int m, n;
    ...
    void dfs(vector<vector<int> >& mat, vector<vector<bool> >& ocean, int i, int j){        
        if(ocean[i][j]) return;
        ocean[i][j] = true;
...
/*⬇️*/  if(i + 1 <  m && mat[i + 1][j] >= mat[i][j]) dfs(mat, ocean, i + 1, j); 
/*⬆️*/  if(i - 1 >= 0 && mat[i - 1][j] >= mat[i][j]) dfs(mat, ocean, i - 1, j);
/*➡️*/  if(j + 1 <  n && mat[i][j + 1] >= mat[i][j]) dfs(mat, ocean, i, j + 1); 
/*⬅️*/  if(j - 1 >= 0 && mat[i][j - 1] >= mat[i][j]) dfs(mat, ocean, i, j - 1);
    }
};